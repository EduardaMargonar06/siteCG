...
document.addEventListener('mousemove', (e) => {
    ...
    if (!throttle) {
        ...
        // Isso evita criar centenas de brilhos por segundo e travar o site
        setTimeout(() => {
            throttle = false;
        }, 50); // Ajuste esse valor (30-100) como preferir
    }
});
...
function createSparkle(x, y) {
    ...
    // Isso será usado pelas variáveis CSS (--x, --y) na animação
    ...
    // Remove o brilho da página depois que a animação terminar (1000ms = 1s)
    // Isso é MUITO importante para não encher a página de elementos
    setTimeout(() => {
        sparkle.remove();
    }, 1000);
}